...
class Stage extends FlxTypedGroup<FlxBasic>
{
	...
	public function new(curStage)
	{
		...
		switch (curStage)
		{
			...
			case 'wall':
				...
				var bg:FlxSprite = new FlxSprite(-640, -360).loadGraphic(Paths.image('backgrounds/' + curStage + '/wall'));
				bg.antialiasing = true;
				...
				flashlight.antialiasing = true;
				flashlight.scrollFactor.set(0, 0);
				flashlight.blend = BlendMode.MULTIPLY;
	...
	public function dadPosition(curStage, dad:Character, gf:Character, camPos:FlxPoint):Void
	{
	...
	public function repositionPlayers(curStage, boyfriend:Character, opponentArray:Array<Character>, gf:Character):Void
	{
		...
		switch (curStage)
		{
			...
			case 'wall':
				boyfriend.x -= 345;
				boyfriend.y -= 115;
				opponentArray[0].x -= 160;
				opponentArray[0].y -= 115;

				if (opponentArray.length > 1)
				{
					opponentArray[1].x -= 500;
					opponentArray[1].y += 175;

					opponentArray[2].flipX = false;
					opponentArray[2].x += 700;
					opponentArray[2].y += 15;
				}
			...
			case 'wallEvil':
				boyfriend.x -= 345;
				boyfriend.y -= 115;
				opponentArray[0].x -= 560;
				opponentArray[0].y -= 175;
	...
	public function stageUpdate(curBeat:Int, boyfriend:Boyfriend, gf:Character, opponentArray:Array<Character>)
	{
	...
	public function stageUpdateConstant(elapsed:Float, boyfriend:Boyfriend, gf:Character, opponentArray:Array<Character>)
	{
		switch (PlayState.curStage)
		{
			case 'philly':
