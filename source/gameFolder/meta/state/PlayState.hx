...
class PlayState extends MusicBeatState
{
	...
	public static var opponentArray:Array<Character>;
	...
	override public function create()
	{
		...
		// i might un-hardcode this eventually but for now its gonna be a lil unclean
		switch (PlayState.SONG.song.toLowerCase())
		{
			case 'slaughter-me-street' | 'lurk-inside':
				opponentArray = [
					new Character(100, 100, 'follower'),
					new Character(100, 100, 'greeter'),
					new Character(100, 100, 'waiter')
				];
			default:
				opponentArray = [new Character(100, 100, SONG.player2)];
		}
		...
		for (opponentChar in opponentArray)
		{
			stageBuild.dadPosition(curStage, opponentChar, gf, camPos);
		}
		...
		stageBuild.repositionPlayers(curStage, boyfriend, opponentArray, gf);
	...
	{
		stageBuild.stageUpdateConstant(elapsed, boyfriend, gf, opponentArray);
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			{
				var char = opponentArray[opponentIndex];

				var getCenterX = char.getMidpoint().x + 150;
				var getCenterY = char.getMidpoint().y - 100;
				switch (opponentArray[opponentIndex].curCharacter)
				{
					case 'mom':
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.mustPress)
					...
					mainControls(daNote, opponentArray[opponentIndex], dadStrums,
						dadAutoplay); // dadOpponent autoplay is true by default and should be true unless neccessary
				// */
	...
	function characterPlayAnimation(coolNote:Note, character:Character)
	{
		...
		if (character.curCharacter == 'abomination')
		{
			camera.shake(0.005, 0.15);
		}
	}
	...
	override function stepHit()
	{
		...
		if (PlayState.SONG.song.toLowerCase() == 'slaughter-me-street')
		{
			// waiter cum
			if (curStep == 534)
			{
	...
	override function beatHit()
	{
		...
		// chooses who to sing cuz i dont wanna edit the base fnf chart type lol
		if (PlayState.SONG.song.toLowerCase() == 'slaughter-me-street')
		{
			// follower
			if (curBeat == 48)
		...
		if (PlayState.SONG.song.toLowerCase() == 'lurk-inside')
		{
			// follower
			if (curBeat == 144)
			{
		...
		// stage stuffs
		stageBuild.stageUpdate(curBeat, boyfriend, gf, opponentArray);
	}
