...
class MainMenuState extends MusicBeatState
{
	...
	var maxTimerCounter:Float = 0.168;
	...
	override function create()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			{
				menuItem.animation.addByPrefix('start_begin', "begin", 24, false);
			}
	...
	override function update(elapsed:Float)
	{
		...
		{
			animTimer += elapsed;
			if (animTimer > maxTimerCounter && curSelected == 0)
			{
				onTimerComplete();
				animTimer = 0;
			}
	...
	private function updateSelection()
	{
		...
		if (menuItems.members[Math.floor(curSelected)].animation.curAnim.name == 'idle')
			if (curSelected == 0)
			{
				menuItems.members[Math.floor(curSelected)].animation.play('start_begin');
				animTimer = 0.01;
			}
			else
			{
	...
	{
		menuItems.members[0].animation.play('selected');
	}
}
